    ...
    try:
        while True:
            ...
            for record in records:
                ...
                if record.error():
                    # Error or event
                    if record.error().code() == KafkaError._PARTITION_EOF:sys.stderr.write('%% {} [{}] reached end at offset {} - {}\n'.format(record.topic(),record.partition(),record.offset()))
...
if __name__ == '__main__':
    while True:
        try:
            red_green = [item_consume_red(), item_consume_green()]
            for item in red_green:
                if item == item_consume_red():
                    item_consume_red()
                    print("black tea consume success!")
                elif item == item_consume_green():
                    item_consume_green()
                    print("green tea consume success!")

            print("Next step---camera_capture---")
